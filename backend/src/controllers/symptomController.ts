...
export const analyzeSymptoms = async (req: AuthRequest, res: Response) => {
    try {
        ...
        You MUST respond ONLY with a valid JSON object in this exact format (do not include markdown tags like \`\`\`json):
        {
            ...
            "possible_conditions": [
                {"condition": "<string>", "probability": <number between 0.0 and 1.0>}
            ],
            "precautions": [
                "<string, actionable precaution 1>",
                "<string, actionable precaution 2>"
            ],
            "doctor_advice": "<string, professional recommendation on whether/when to see a doctor>",
            "ai_analysis_notes": "<string, a short compassionate 2-3 sentence paragraph explaining the overall assessment>"
        }`;
        ...
        try {
            ...
            console.error("Gemini AI API Error in symptoms fallback logic:", geminiErr.message);
            // Fallback for API Key 403 or other failure
            aiAnalysis = {
                risk_score: 45,
                severity: "Medium Severity",
                possible_conditions: [
                    { condition: "Condition based on symptoms (Fallback)", probability: 0.75 }
                ],
                precautions: [
                    "Rest and stay hydrated.",
                    "Monitor your symptoms and seek medical help if they worsen."
                ],
                doctor_advice: "Since AI generation is currently unavailable, please consult a healthcare professional for an accurate diagnosis.",
                ai_analysis_notes: "The AI service is temporarily unavailable. We generated a default baseline risk score based on generic triage guidelines."
            };
        }
